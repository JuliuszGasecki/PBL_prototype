...
public class Climbing : MonoBehaviour
{
    ...
    private void Start()
    {
        girl = GameObject.Find("Boi");
        isCrawling = false;
    }
    private void Update()
    {
        int pointsValue = checkIfStarts();
        if(pointsValue != -1 && !isCrawling && teleportPoints[0].GetComponent<InformClimbing>().isReady && teleportPoints[1].GetComponent<InformClimbing>().isReady)
        {
            Crawle(pointsValue);
            isCrawling = true;
        }
        if (isCrawling)
        {
            girl.GetComponent<ControlByNUM>().blockControlls();
        ...
        {
            girl.GetComponent<ControlByNUM>().freeControlls();
            isCrawling = false;
        }
    }
    ...
    {
        if (teleportPoints[0].GetComponent<InformClimbing>().isStartPoint && teleportPoints[0].GetComponent<InformClimbing>().isReady)
        {
            return 0;
        }
        else if (teleportPoints[1].GetComponent<InformClimbing>().isStartPoint && teleportPoints[1].GetComponent<InformClimbing>().isReady)
        {
            return 1;
        }
        else return -1;
